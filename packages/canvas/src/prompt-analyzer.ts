...
const formatVector = (vector?: Vector3D) => {
  if (!vector) {
    return "未设置"
  }
...
export function formatCameraPath(path: Vector3D[] = []) {
  if (path.length === 0) {
...
export function formatObjectBlocking(objects: Previs3DObject[]) {
  if (objects.length === 0) {
...
export function analyzeStoryboard(storyboard: StoryboardFrameContent): StoryboardAnalysis {
  const shotType = storyboard.shotType ?? "medium"
  const cameraMovement = storyboard.cameraMovement ?? "static"
...
export function composeShotDescription(pipeline: CinematicPromptPipelineResult) {
...
export function composeCinematicPrompt(pipeline: CinematicPromptPipelineResult, storyboard: StoryboardFrameContent): PromptAnalysisResult {
...
export function runCinematicPromptPipeline(storyboard: StoryboardFrameContent, previs: Previs3DSceneContent): PromptAnalysisResult {
...
export function analyzePromptFromStoryboard(storyboard: StoryboardFrameContent, previs: Previs3DSceneContent): PromptAnalysisResult {
  return runCinematicPromptPipeline(storyboard, previs)
}
