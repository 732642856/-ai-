...
import { X, Plus, UserRound, Trash2, Image as ImageIcon, Save, Sparkles, Loader2, Zap } from "lucide-react"
import { DESIGN_TOKENS, ICON_CONFIG } from "../../styles/designSystem"
import { useCanvasStore } from "../../stores/canvasStore"
import type { CharacterBibleData } from "../canvas/types"
import { generateId } from "../../utils/generateId"
import { buildCharacterIdentitySummary } from "@/lib/storyboard/characterIdentitySummary"
import { enhancePromptToIdeogram } from "@/lib/ai/shot-to-ideogram-prompt"
import { registerCharacterAsset } from "@/lib/storyboard/characterAssetLibrary"
...
export function CharacterBiblePanel({ isOpen, onClose }: CharacterBiblePanelProps) {
  ...
  const handleGenerateCharacterImage = async () => {
    if (!selectedBibleCharacterId || imageGenerating) return
    const char = bibleCharacters.find((c) => c.id === selectedBibleCharacterId)
    if (!char) return
    ...
    try {
      // 1. 构建角色描述
      const summary = buildCharacterIdentitySummary(char)
      if (!summary.trim()) {
        throw new Error("角色信息不足，请至少填写姓名和外貌特征后再生成")
      }

      // 2. 构建 Ideogram 4 JSON 提示词
      const payload = enhancePromptToIdeogram(summary, {
        aspectRatio: "1x1",
        lighting: "soft studio lighting, professional portrait lighting, cinematic quality",
        colorPalette: char.colorPalette,
      })

      // 3. 调用 Ideogram API 生成角色形象
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || "生成角色形象失败")
      const imageUrl = data.imageUrl
      if (!imageUrl) throw new Error("未返回图片 URL")

      // 4. 保存图片 URL 到角色的 referenceImageUrl
      updateBibleCharacter(selectedBibleCharacterId, { referenceImageUrl: imageUrl })
      setEdit((prev) => ({ ...prev, referenceImageUrl: imageUrl }))

      // 5. 注册到资产库
      registerCharacterAsset({ ...char, referenceImageUrl: imageUrl }, addAsset)
    } catch (err: any) {
      console.error("AI generate character image failed:", err)
      setImageError(err.message || "生成角色形象失败")
    } finally {
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-lg px-2.5 py-1 text-[10px] font-medium transition-colors disabled:opacity-50"
                  ...
                  >
                    ...
                    {imageGenerating ? "生成中..." : "AI 生成形象"}
                  </button>
                </div>

                {/* 错误提示 */}
                {imageError && (
                  <div className="mb-2 rounded-lg px-2.5 py-1.5 text-[11px]" style={{ backgroundColor: "rgba(239, 68, 68, 0.1)", color: "rgb(248, 113, 113)" }}>
                    {imageError}
                  </div>
                )}
                ...
                >
                  {edit.referenceImageUrl ? (
                    <div className="relative w-full">
                      ...
                      />
                      <button
                        onClick={(e) => { e.stopPropagation(); setEdit((prev) => ({ ...prev, referenceImageUrl: undefined })); setImageError(null) }}
