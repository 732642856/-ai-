    ...
    const callRes = await fetch(`${HF_SPACE_URL}/call/generate`, {
      ...
      body: JSON.stringify({
        data: [
          ...
          // Voice Design instruct (optional, appended as 16th param if provided)
...
// ---------------------------------------------------------------------------
// Unified voice generation API
// ---------------------------------------------------------------------------

export interface GenerateVoiceInput {
  text: string
  /** Voice profile ID for clone mode, or omit/null for design/auto mode */
  profileId?: string | null
  /** Voice design instruct (e.g. "Female, Young Adult, speaking gently") */
  instruct?: string
  /** Emotion / tone override */
  emotion?: string
  speed?: number
  language?: string
  numStep?: number
...
/**
 * Unified voice generation — supports both clone and design modes.
 *
 * - If `profileId` is provided → clone mode (self-hosted backend)
 * - If `instruct` is provided  → design mode (OmniVoice HF Space)
 * - Otherwise                  → auto mode
 */
export async function generateVoice(input: GenerateVoiceInput): Promise<GenerateVoiceResult> {
  const { text, profileId, instruct, emotion, speed, language, numStep } = input
  ...
  // Build full instruct with emotion appended
  let fullInstruct = instruct || ""
  if (emotion) {
    const emotionMap: Record<string, string> = {
      "温柔": "speaking gently",
      "激动": "excited",
    ...
    fullInstruct = fullInstruct ? `${fullInstruct}, ${mappedEmotion}` : mappedEmotion
  }
  ...
  // Design / Auto mode via HF Space
  const arrayBuffer = await callGradioSpaceApi({
    text: text.trim(),
    instruct: fullInstruct || undefined,
    speed,
    numStep,
    language,
  })
...
export interface GenerateVoiceBatchItem {
  text: string
  emotion?: string
}

export interface GenerateVoiceBatchResult {
  items: GenerateVoiceResult[]
  errors: { index: number; message: string }[]
}

/**
 * Batch voice generation — clone mode only (uses self-hosted backend batch API).
 */
export async function generateVoiceBatch(
  profileId: string,
  lines: GenerateVoiceBatchItem[],
  options?: { speed?: number; language?: string },
): Promise<GenerateVoiceBatchResult> {
  if (!profileId) {
    throw new TtsGenerationError({
      message: "批量合成需要提供 voice profile ID",
      code: "MISSING_PROFILE_ID",
      retryable: false,
    })
  }
  ...
  const controller = new AbortController()
  const timeout = setTimeout(() => controller.abort(), TTS_TIMEOUT_MS * Math.max(1, Math.ceil(lines.length / 3)))

  try {
    const resp = await fetch(`${VOICE_CLONE_API_URL}/api/voice-clone/synthesize/batch`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      signal: controller.signal,
      body: JSON.stringify({
    ...
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({ detail: `HTTP ${resp.status}` }))
      throw new TtsGenerationError({
        message: err.detail || `Batch synthesis failed (${resp.status})`,
    ...
    const result = await resp.json()
    const items: GenerateVoiceResult[] = []
    const errors: { index: number; message: string }[] = []

    for (let i = 0; i < result.jobs.length; i++) {
      const job = result.jobs[i]
      if (job.status === "done" && job.audio_base64) {
        const binaryString = atob(job.audio_base64)
        const bytes = new Uint8Array(binaryString.length)
        for (let j = 0; j < binaryString.length; j++) {
          bytes[j] = binaryString.charCodeAt(j)
        }
        const arrayBuffer = bytes.buffer
        const audioBlob = new Blob([arrayBuffer], { type: job.audio_mime || "audio/wav" })
        const audioUrl = URL.createObjectURL(audioBlob)
        const dataSize = arrayBuffer.byteLength - 44
        const durationSeconds = dataSize > 0 ? dataSize / 48000 : 0
        items.push({ audioBlob, audioUrl, durationSeconds })
      } else {
        errors.push({ index: i, message: job.error_message || "Synthesis failed" })
    ...
    if (error?.name === "AbortError") {
      throw new TtsGenerationError({
        message: "批量语音合成超时，请稍后重试",
        code: "BATCH_TIMEOUT",
        retryable: true,
      })
    }
    throw new TtsGenerationError({
      message: `批量语音合成请求失败：${error?.message || "未知错误"}`,
      code: "BATCH_NETWORK_ERROR",
      retryable: true,
    })
  } finally {
    clearTimeout(timeout)
