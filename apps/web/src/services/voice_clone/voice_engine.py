...
class OmniVoiceSpaceBackend(VoiceCloneBackend):
    ...
    async def _poll_result(self, event_id: str, max_retries: int = 60, interval: float = 2.0) -> Optional[list]:
        """Poll SSE endpoint for async result."""
        for _ in range(max_retries):
            resp = await self.client.get(f"{self.HF_SPACE_URL}/call/generate/{event_id}")
