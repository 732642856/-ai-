...
import { EMOTION_SHOT_STRATEGY, createCinematicShotId, runAllChecks } from "./cinematic-rules"
...
// ============================================================================
// Script Analysis Agent (P2-6)
// ============================================================================

export const SCRIPT_ANALYSIS_SYSTEM_PROMPT = `你是一位专业影视分析 AI。请仔细阅读剧本，提取以下三类结构化信息：

## 1. 角色列表 (characters)
- 提取所有有名字或有显著特征的角色。
- 每个角色包含：name（姓名）、role（角色定位如主角/反派/配角）、visualSignature（外貌特征）、costume（服装风格）、props（常用道具数组）、backstory（背景故事）、aliases（别名数组）、physicalTraits（外貌细节数组）、colorPalette（角色关联色彩数组）、arcDescription（人物弧光）。

## 2. 场景列表 (scenes)
- 按地点变化、时间跳跃、叙事功能变化拆分场景。
- 每个场景包含：sceneNumber（场景编号，从1开始）、location（地点）、timeOfDay（时间段）、weather（天气）、atmosphere（氛围描述）、characters（出场角色名数组）、description（场景详细描述）、lightingStyle（光影风格）、colorPalette（场景色调数组）。

## 3. 分镜建议 (shots)
- 为每个场景设计关键镜头，给出分镜建议。
- 每个镜头包含：order（镜头顺序号，全局递增）、dramaticBeat（剧情节拍）、shotPurpose（镜头目的）、emotionalState（情绪状态，如 calm/tense/fear/anger/joy/sadness/intimacy/isolation/suspense/revelation）、shotSize（景别，如 extreme-wide/wide/medium/close-up/extreme-close-up）、cameraAngle（机位，如 eye-level/low-angle/high-angle/over-shoulder/top-shot/dutch-angle）、cameraMovement（运动，如 static/push-in/pull-out/pan/tilt/tracking/handheld/dolly/crane/zoom）、composition（构图描述）、blocking（人物调度）、durationEstimate（预估时长秒数，远景4-6中景2-4近景1-3）、visualPrompt（英文视觉提示词，可直接用于图像模型）、dialogue（关联台词）、voiceIntent（配音意图：语气/语速/情感/潜台词）。
...
export type ScriptAnalysisCharacter = {
  name: string
  role?: string
...
export type ScriptAnalysisScene = {
  sceneNumber: number
  location: string
  timeOfDay?: string
  weather?: string
...
export type ScriptAnalysisShot = {
  order: number
  dramaticBeat?: string
  shotPurpose?: string
...
/**
 * 执行导演代理，分析剧本文本并返回结构化结果。
 */
export async function executeDirectorAgent(script: string): Promise<ScriptAnalysisResult> {
  ...
  const parsed = parseDirectorJson(res.content) as Record<string, unknown>

  return {
    characters: Array.isArray(parsed.characters) ? (parsed.characters as ScriptAnalysisCharacter[]) : [],
    scenes: Array.isArray(parsed.scenes) ? (parsed.scenes as ScriptAnalysisScene[]) : [],
    shots: Array.isArray(parsed.shots) ? (parsed.shots as ScriptAnalysisShot[]) : [],
  }
}
